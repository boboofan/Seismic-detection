...
class CNN(object):
    ...
    def setup_layer(self):
        ...
        layer['input'] = tf.placeholder(tf.float32,
                                        shape=[None, 1, self.config.winsize, 3],
                                        name='input')
        ...
        layer['logits'] = layers.fc(layer['unfold'],
                                    2,
                                    wd=1e-3,
                                    name='logits')
    ...
    def setup_loss(self):
        with tf.name_scope('loss'):
            raw_loss = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.layer['logits'],
...
if __name__ == '__main__':
    cnn = CNN()
    cnn.train(3000, new_training=True)
